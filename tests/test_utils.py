...
from data.config import TEST_TRANSACTION_PATH_CSV, TEST_TRANSACTION_PATH_XLSX, USER_SETTINGS_JSON, \
    TEST_UTILS_FileNotFoundError, \
    TEST_UTILS_JSONDecodeError, TEST_READING_JSON, TEST_USER_SETTINGS_JSON
from src.utils import get_greeting, get_information_on_card, reading_csv_xlsx_file, get_list_dict_json, \
    get_top5_transactions, get_user_stocks, get_user_currencies
...
def test_get_top5_transactions(str_datetime):
    assert get_top5_transactions(str_datetime[0]) == [{'amount': 20000.0,
                                                       'category': 'Другое',
                                                       'date': '2021-12-23 16:45:12',
                                                       'description': 'Иван С.'},
                                                      {'amount': 3500.0,
                                                       'category': 'Пополнения',
                                                      ...
                                                      {'amount': -15.0,
                                                       'category': 'Косметика',
                                                       'date': '2021-12-09 16:03:32',
                                                       'description': 'Улыбка радуги'},
                                                      {'amount': -100.0,
                                                       'category': 'Переводы',
                                                       'date': '2021-12-09 01:07:48',
                                                       'description': 'Перевод на карту'},
                                                      {'amount': -837.9,
                                                       'category': 'Ж/д билеты',
                                                       'date': '2021-12-07 14:02:27',
                                                       'description': 'РЖД'},
                                                      {'amount': 1721.38,
                                                       'category': 'Каршеринг',
                                                       'date': '2021-12-12 15:03:30',
                                                      ...
                                                      {'amount': -1.0,
                                                       'category': 'Каршеринг',
                                                       'date': '2021-12-17 23:40:14',
                                                       'description': 'Ситидрайв'},
                                                      {'amount': -1.07,
                                                       'category': 'Каршеринг',
                                                       'date': '2021-12-01 23:40:34',
                                                       'description': 'Ситидрайв'},
                                                      {'amount': 421.0,
                                                       'category': 'Различные товары',
                                                       'date': '2021-12-20 19:42:13',
                                                       'description': 'Ozon.ru'},
                                                      {'amount': 180.77,
                                                       'category': 'Каршеринг',
                                                       'date': '2021-12-03 19:05:50',
                                                       'description': 'Ситидрайв'},
                                                      {'amount': 15.0,
                                                       'category': 'Другое',
                                                       'date': '2021-12-21 22:57:26',
                                                       'description': 'Google'},
                                                      {'amount': -5.7,
                                                       'category': 'Каршеринг',
                                                       'date': '2021-12-25 15:30:02',
                                                       'description': 'Ситидрайв'},
                                                      {'amount': -7.94,
                                                       'category': 'Каршеринг',
                                                       'date': '2021-12-21 17:21:58',
                                                       'description': 'Ситидрайв'}]
    with pytest.raises(ValueError):
...
def test_get_user_stocks(user_settings_stock):
    with patch('requests.get') as mock_get:
        mock_get.return_value.json.return_value = {'open': 194.45, 'high': 194.76, 'low': 192.12, 'close': 192.32,
                                                   'volume': 39500106.0, 'adj_high': 194.76, 'adj_low': 192.11,
                                                   'adj_close': 192.32, 'adj_open': 194.45, 'adj_volume': 40895115.0,
                                                   'split_factor': 1.0, 'dividend': 0.0, 'symbol': 'AAPL',
                                                   'exchange': 'XNAS', 'date': '2023-12-06T00:00:00+0000'}
        assert get_user_stocks(user_settings_stock) == [{'price': 192.32, 'stock': 'AAPL'}]
        mock_get.assert_called_with('http://api.marketstack.com/v1/tickers/AAPL/eod/latest',
...
def test_get_user_currencies(mock_get, user_settings_currencies):
    mock_get.return_value.json.return_value = {'base': 'USD', 'date': '2023-12-07', 'rates': {'RUB': 92.42499},
                                               'success': True, 'timestamp': 1701981182}

    assert get_user_currencies(user_settings_currencies) == [{'currency': 'USD', 'rate': 92.42499}]
    mock_get.assert_called_with('https://api.apilayer.com/fixer/latest?symbols=RUB&base=USD',
